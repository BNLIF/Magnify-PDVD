# Magnify-protodune: Input File Format

The viewer consumes a ROOT file produced in two stages: first Wire-Cell Toolkit (WCT) writes per-readout-unit (APA) histograms, then `preprocess.sh` merges them into a single whole-detector file that the viewer actually opens.

---

## Stage A — Wire-Cell Output (per-APA file)

This is the file you feed to `preprocess.sh`. Wire-Cell writes one or more histogram "tags" per APA, named with the APA index appended as a suffix (`0` … `5` for the 6 ProtoDUNE-HD APAs).

### 2-D Histograms (TH2)

X axis = channel id (continuous across all channels in that APA, e.g. 0–2559 for APA 0).  
Y axis = tick number (0–5999 for 6000-tick readouts).

| Name pattern | ROOT type | Meaning |
|---|---|---|
| `hu_orig{N}`, `hv_orig{N}`, `hw_orig{N}` | `TH2I` | Raw ADC (before any noise filter); U, V, W planes of APA N |
...
|---|---|---|
| `T_bad{N}` | `chid` (int), `start_time` (int), `end_time` (int) | Bad-channel regions for APA N. One entry per contiguous bad interval. |
...
### Default histogram dimensions (ProtoDUNE-HD)

| Parameter | Value | Source |
|---|---|---|
| Number of channel bins | 15360 | 6 APAs × 2560 ch/APA |
| Channel axis range | −0.5 … 15359.5 | `preprocess.C:108-109` |
| Number of tick bins | 6000 | |
| Tick axis range | 0 … 6000 | `preprocess.C:110-111` |
...
X axis = global channel id (0 … 15359).  
Y axis = tick number (0 … 5999).
