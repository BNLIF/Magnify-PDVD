# Porting Magnify to ProtoDUNE Vertical Drift (VD)

This document catalogues every ProtoDUNE-HD-specific assumption baked into the current code, with the exact file and line number where it appears. Use it as a change-checklist when adapting the package to ProtoDUNE-VD.

**Rule of thumb**: before touching any source file, decide the VD geometry numbers (see §1) and record them in a new `event/Geometry.h` (see §12 — Suggested Porting Order). Then replace each hard-coded constant listed below with a reference to that header.
